...
A harness being **detected** is independent of whether the installer will **wire it up**. Today, `agentlock install` only wires Claude Code and Codex CLI. The other detectors land in the picker but the installer flags them as not yet implemented and skips writing hook entries.
